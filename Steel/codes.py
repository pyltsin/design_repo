...
class snipn(normes):
    ...
    def local_buckl_h_ne_old(self, typ, typ1):
        ...
        if (typ == 1 and (mx < 0 or mx > 0) and self.c()[0] * self.phiy() <= self.phi_e(1)[0]):
            ...
            e = self.el.steel.e()
            sigma = self.force.n / self.pr.a() + self.force.mx / self.pr.jx() * (
                self.pr.h() / 2 - self.pr.t1() - self.pr.t2())
            sigma1 = self.force.n / self.pr.a() - self.force.mx / self.pr.jx() * (
                self.pr.h() / 2 - self.pr.t1() - self.pr.t2())
            ...
            if a > 0.5:
                lambda_uw1 = 4.35 * ((2 * a - 1) * e / (2 - a + (a ** 2 + 4 * beta ** 2) ** 0.5) / sigma) ** 0.5 * (
                                                                                                                       ry / e) ** 0.5
    ...
    def local_buckl_h_ne(self, typ, typ1, typ2=1, typ3=0):
        ...
        if (self.pr.title() == 'korob' and typ3 != 2) or (
                            self.pr.title() == 'dvut' and typ == 1 and self.c()[0] * self.phiy() > self.phi_e(1)[
                    0]) or typ3 == 1:
            #            print 'tut1'
            #            if typ==1 or typ3==1:
            if typ == 1:
        ...
        if (self.pr.title() == 'dvut' and (
                        typ == 2 or (typ == 1 and self.c()[0] * self.phiy() <= self.phi_e(1)[0]))) or typ3 == 2:
            lambda_w = self.pr.hef() / self.pr.s() * (self.element.steel.ry() / self.element.steel.e()) ** 0.5
            ...
            if (typ == 1 and self.c()[0] * self.phiy() <= self.phi_e(1)[0]) or typ3 == 2:
                ...
                if typ == 1:
                    sigma1 = self.force.n / self.pr.a() + self.force.mx / self.pr.jx() * (
                        self.pr.h() / 2 - self.pr.t1() - self.pr.t2())
                    sigma2 = self.force.n / self.pr.a() - self.force.mx / self.pr.jx() * (
                        self.pr.h() / 2 - self.pr.t1() - self.pr.t2())
                    tau = self.force.qx / (self.pr.s() * (self.pr.hw()))
                if typ == 2:
                    sigma1 = self.force.n / self.pr.a() + self.force.my / self.pr.jy() * (
                        self.pr.b() / 2 - self.pr.t1() - self.pr.t2())
                    sigma2 = self.force.n / self.pr.a() - self.force.my / self.pr.jy() * (
                        self.pr.b() / 2 - self.pr.t1() - self.pr.t2())
    ...
    def local_buckl_h_ne_ut(self, typ, typ1, typ2=1):
        ...
        if (typ == 1 and self.pr.title() == 'dvut') or self.pr.title() == 'korob':
            if self.c()[0] * self.phiy() > self.phi_e(1)[0] or self.pr.title() == 'korob':
                ...
                k = self.force.n / (
                    self.phi_e(typ=typ)[0] * self.element.steel.ry() * self.yc() * self.element.profile.a())
    ...
    def local_buckl_b_ne(self, typ, typ2=1):
        ...
        if self.pr.title() == 'korob' or (self.pr.title() == 'dvut' and typ == 1):
            ...
            if typ == 2:
                ...
                # нет тестов вот тут
            if self.pr.title() == 'dvut':
                if self.c()[0] * self.phiy() <= self.phi_e(1)[0] or self.force.n / (
                                    self.pr.a() * self.phi_e(1)[0] * self.element.steel.ry() * self.yc()) < 0.8:
            ...
            if self.pr.title() == 'korob':
                if self.phi() <= self.phi_e(1)[0] or self.force.n / (
                                    self.pr.a() * self.phi_e(1)[0] * self.element.steel.ry() * self.yc()) < 0.8:
                    lambda_uf20 = self.local_buckl_b_m()[1]
                else:
                    if typ2 == 1:
...
class beam(snipn):
    ...
    def output_data_snip_old_global(self, typ, typ1, typ2, typ3):
        ...
        if self.pr.title() != 'korob':
            ...
            if self.element.lb() < self.element.lfact() and self.element.lb() != 0:
                n3 = (self.pr.b() * self.pr.t() + 0.25 * (
                    self.pr.h() - 2 * self.pr.t()) * self.pr.s()) * self.element.steel.ry()
                lambda_ = self.element.lb() / (self.pr.b() / 12. ** 0.5) * (
                                                                               self.element.steel.ry() / self.element.steel.e()) ** 0.5
                phi = self.phi_n_old(lambda_)
    ...
    def mxb_old(self, typ, typ1, typ2, typ3):
        ...
        mxb = self.pr.wx() * self.element.steel.ry() * self.ycb() * self.phi_b_old(typ, typ1, typ2, typ3)[
            0] / 100. / 1000.
        return mxb
...
class FermaPP(ferma):
    ...
    def outDataN(self, lambdaP, lambdaML, lambdaM):
        ...
        # Организуем список № 2 и заодно находим общий плохой случай и по п.
        lst2 = []
        lst2Header = [u'Усилия, т', u'КиспMax', u'Kисп+ (п.7.1.1,2)', u'Kисп- (п.7.1.3.(7))', u'Гибкость +',
                      u'Гибкость -']
...
class BeamPP(beam):
    ...
    def outDataOld(self, typ, typ1, typ2, typ3):
        ...
        # Организуем список № 2 и заодно находим общий плохой случай и по п.
        lst2 = []
        lst2Header = [u'Mx, т*м', u'My, т*м', u'Qx, т', u'Qy, т', u'КиспMax', u'Kпр (п.5.17 (38))',
                      u'Kпр (п.5.12 (29))', u'Kпр (п.5.14 (33))', u'Kуст (п.5.15 (34))']
        ...
        for i in self.force.lstForce:
            ...
            sx = abs(mx / mxult * self.element.steel.ry() * self.yc()) + abs(
                my / myult * self.element.steel.ry() * self.yc())
            sy = 0
            tx = qx / qxult * self.element.steel.rs() * self.yc()
            ty = qy / qyult * self.element.steel.rs() * self.yc()

            kpr33 = (sx ** 2 + sy ** 2 - sx * sy + 3 * tx ** 2 + 3 * ty ** 2) ** 0.5 / (
                1.15 * self.element.steel.ry() * self.yc())
    ...
    def outDataN(self, typ, typ1, typ2, typ3):
        ...
        # Организуем список № 2 и заодно находим общий плохой случай и по п.
        lst2 = []
        lst2Header = [u'Mx, т*м', u'My, т*м', u'Qx, т', u'Qy, т', u'КиспMax', u'Kпр (п.8.2.1 (43))',
                      u'Kпр (п.8.2.1 (42))', u'Kпр (п.8.2.1 (44))', u'Kуст (п.8.4.1 (70))']
...
k29,k33,k50,k34,k7x,k7y, k51x,k51y,k56,k62,kLambdaP, kLambdaM           
        ...
        lst11 = max(l44s, l44n, l106, l70, l7x, l7y, l109x, l109y, l111, l116, l120, l121, lLambdaP, lLambdaM, lst16,
                    lst17)
        #        print lst11, 'lst11'
        #        print lst12
        #        print lst13
        ...
        #        print lst16
        #        print lst17
        lst1 = [lst11, l44s, l44n, l106, l70, l7x, l7y, l109x, l109y, l111, l116, l120, l121, lLambdaP, lLambdaM, lst16,
                lst17]
